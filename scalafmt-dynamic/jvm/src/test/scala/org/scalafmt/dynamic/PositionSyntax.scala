...
object PositionSyntax {
  ...
  def formatMessage(pos: Position, severity: String, message: String): String =
    pos match {
      case Position.None => s"$severity: $message"
      case _ => new java.lang.StringBuilder().append(pos.lineInput)
          .append(if (severity.isEmpty) "" else " ").append(severity).append(
            if (message.isEmpty) ""
  ...
  implicit class XtensionPositionsScalafix(private val pos: Position)
    ...
    private def lineContent: String = pos match {
      case Position.None => ""
      case range: Position.Range => lineContent(range.startLine).text
    }
  }
