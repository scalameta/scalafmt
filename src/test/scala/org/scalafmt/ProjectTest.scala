...
object ProjectTest extends App {
  ...
  case class UnknownFailure(url: String,
      e: Throwable) {

    override def toString: String = s"$url $e"
  }

  case class ParseErr(url: String,
      e: ParseException) {
  ...
  def parse(filename: String): Future[Boolean] =
    Future {
      ...
      if (!ScalacParser.checkParseFails(code))
        {
          ...
          successes.add(ParseSuccess(filename,
                                     System.currentTimeMillis() - startTime))
        }
      else { skipped.add(filename) }
    }

  def checkRepo(url: String, filter: String => Boolean = _ =>
          true) = {
  ...
  def scalaIgnore: String => Boolean =
    x => !Seq(
    ...
    "target/repos/scala/test/disabled/", "target/repos/scala/test/files/neg/",
  ...
  def run(): Unit = {
    ...
    println(
        f"Successes: ${successes.length}, avg time to parse a single file $avgTime%2.2f ms")
