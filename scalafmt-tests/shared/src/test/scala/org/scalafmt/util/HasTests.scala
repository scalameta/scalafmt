...
trait HasTests extends FormatAssertions {
  ...
  def parseDiffTests(filename: String): Seq[DiffTest] = {
    val absFilename = Paths.get(filename).normalize().toString()
    val content = FileOps.readFile(filename)
    val sep =
      if (content.contains(System.lineSeparator)) System.lineSeparator else "\n"
    val spec = BuildInfo.resourceDirectory.toPath.normalize()
      .relativize(Paths.get(absFilename)).getName(0).toString
    ...
    def loadStyle(cfg: String, base: ScalafmtConfig, ln: Int): ScalafmtConfig =
      ScalafmtConfig.fromHoconString(cfg, base).getOrRecover { c =>
        throw new IllegalArgumentException(
          s"""|Failed to parse line=$ln filename $absFilename:
              |$cfg
              |$c""".stripMargin,
        )
      }
  ...
  private def spec2style(spec: String): ScalafmtConfig = spec match {
    ...
    case "scala3" => ScalafmtConfig.default
        .withDialect(NamedDialect.scala3, "scala3")
    case _ => ScalafmtConfig.default.withDialect(defaultDialect)
  }
...
object HasTests {
  ...
  private val testing = ScalafmtConfig.default.copy(
    ...
    runner = ScalafmtConfig.default.runner.conservative
      .withDialect(defaultDialect),
  )
