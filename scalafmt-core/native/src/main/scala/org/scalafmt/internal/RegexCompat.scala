...
object RegexCompat {
  ...
  val srcLine =
    Pattern.compile(
      RegexCompat.fixHorizontalSpaceInRegex("^/\\/\\/*\\h*(.*?)\\h*$")
    )

  @inline
  val slcDelim =
    Pattern.compile(RegexCompat.fixHorizontalSpaceInRegex("\\h+"))
  ...
  def splitByBeforeTextMatching(
    ...
    while (matcher.find()) {
      ...
      val end = matcher.end()
      if (start != 0)
        res.append(baseText.substring(currPosition, start))
