...
object DanglingParentheses {
  ...
  implicit val decoder: ConfDecoder[DanglingParentheses] = ConfDecoder.instance[DanglingParentheses] {
  ...
  implicit val encoder: ConfEncoder[DanglingParentheses] = ConfEncoder.instance[DanglingParentheses] {
    case DanglingParentheses(true, true) => Conf.Bool(true)
    case DanglingParentheses(true, false) => Conf.Obj("callSite" -> Conf.Bool(true), "defnSite" -> Conf.Bool(false))
    case DanglingParentheses(false, true) => Conf.Obj("callSite" -> Conf.Bool(false), "defnSite" -> Conf.Bool(true))
