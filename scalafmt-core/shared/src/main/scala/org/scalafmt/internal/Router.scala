...
class Router(formatOps: FormatOps) {
    ...
    ft match {
      ...
      case FormatToken(open @ LeftParenOrBracket(), right, _)
        ...
        val noSplit =
          if (nlOnly) Split.ignored
            ...
            val slbOpt =
              if (slbPolicy.nonEmpty) sjsExclude.startOfFirstRange() else None
            val opt = slbOpt.orElse(nextComma)
              .getOrElse(endOfSingleLineBlock(closeFt))
            ...
            val noSplit =
              if (style.newlines.keepBreak(newlines)) Split.ignored
                ...
                Split(Space, 0, policy = slbPolicy ==> onelinePolicy)
                  .withOptimalToken(
                    sjsExclude.startOfFirstRange().getOrElse(end),
                    killOnFail = true,
                  )
              }
            Seq(noSplit, nlSplit)
      ...
      case FormatToken(_: T.KwElse, _, _) if (leftOwner match {
        ...
        if (style.newlines.getBeforeMultiline eq Newlines.unfold)
          ...
          .get(body)(Split(Space, 0).withSingleLine(expire))(nlSplitFunc)
  ...
  private def getSplitsValEqualsClassic(
    ...
    def twoBranches(implicit fileLine: FileLine) = {
      ...
      baseSpaceSplit.withOptimalToken(
        exclude.startOfFirstRange().getOrElse(optimal),
        killOnFail = false,
      ).withPolicy(policyWithExclude(exclude, Policy.End.On, Policy.End.After)(
