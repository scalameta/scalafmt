...
object Cli {
  ...
  case class FileContents(filename: String, override val code: String) extends InputMethod(code)
  ...
  lazy val parser = new scopt.OptionParser[Config]("scalafmt") {
    ...
    opt[Seq[String]]("alignTokens") action { (tokens, c) =>
      val alignsTokens = tokens.map { token =>
        ...
        if (splitted.length != 2) throw new IllegalArgumentException("align token must contain ;")
  ...
  def getCode(config: Config): Seq[InputMethod] = {
    if (config.files.isEmpty) {
      val contents = scala.io.Source.fromInputStream(System.in).getLines().mkString("\n")
      Seq(StdinCode(contents))
    } else {
      config.files.flatMap { file =>
        FileOps.listFiles(file).withFilter(_.endsWith(".scala")).map { filename =>
  ...
  def getConfig(args: Array[String]): Option[Config] = {
    parser.parse(args, Config.default) match {
      case Some(c) if c.configFile.isDefined =>
        parseConfigFile(FileOps.readFile(c.configFile.get)).map(x => c.copy(style = x.style))
      case x => x
    }
  }
