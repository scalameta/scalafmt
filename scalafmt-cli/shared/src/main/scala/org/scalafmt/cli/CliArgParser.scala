  ...
  val scoptParser: OptionParser[CliOptions] =
    new scopt.OptionParser[CliOptions]("scalafmt") {
      ...
      opt[Unit]("no-progress-bar")
        .action((_, c) => c.copy(progressBarMode = ProgressBarMode.Silent))
        .text("disable progress bar")
      opt[Unit]("non-interactive")
        .action((_, c) => c.copy(progressBarMode = ProgressBarMode.Append))
        .text("append progress rather than overlay, useful in CI")
      opt[Unit]("list")
